...
from inkfeed.output.markdown import write_markdown
...
class TestWriteMarkdown:
    def test_creates_index_md(self, tmp_path: Path) -> None:
        ...
        write_markdown("test_source", articles, tmp_path)
    ...
    def test_creates_article_md_files(self, tmp_path: Path) -> None:
        articles = [_make_article("My Article")]
        write_markdown("test_source", articles, tmp_path)

        md_files = list(tmp_path.glob("*.md"))
        assert len(md_files) == 2  # index + 1 article
        article_files = [f for f in md_files if f.name != "index.md"]
    ...
    def test_article_file_contains_content(self, tmp_path: Path) -> None:
        ...
        article_files = [f for f in tmp_path.glob("*.md") if f.name != "index.md"]
    ...
    def test_index_links_to_article_files(self, tmp_path: Path) -> None:
        ...
        article_files = [f.name for f in tmp_path.glob("*.md") if f.name != "index.md"]
        for fname in article_files:
            assert fname in index_content
    ...
    def test_image_references_preserved(self, tmp_path: Path) -> None:
        html = '<p>Look: <img src="images/abc123.png" alt="photo"></p>'
        articles = [_make_article(content_html=html)]
        write_markdown("test_source", articles, tmp_path)

        article_files = [f for f in tmp_path.glob("*.md") if f.name != "index.md"]
        content = article_files[0].read_text()
        assert "images/abc123.png" in content
    ...
    def test_multiple_articles_numbered(self, tmp_path: Path) -> None:
        ...
        md_files = list(tmp_path.glob("*.md"))
        assert len(md_files) == 6  # index + 5 articles
